...
Функция ПолучитьИсключенныхПользователей(ИнформационнаяБаза) Экспорт
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	ИсключаемыеПользователи.Пользователь КАК Пользователь,
			|	ИсключаемыеПользователи.ПользовательКлючПоиска КАК ПользовательКлючПоиска
			|ИЗ
...
Функция ПолучитьРаботающих(ИнформационнаяБаза,ТекстОшибки = "") Экспорт
	фПервая = Истина;
...
Функция ПолучитьПользователейИБ(ИнформационнаяБаза,ТекстОшибки = "") Экспорт
	СткЗапроса = ОбщегоНазначения.ПолучитьСткЗапроса();
	ЗаполнитьЗначенияСвойств(СткЗапроса,ИнформационнаяБаза);
	СткЗапроса.ИмяМетода = "USERSLIST";
	ОшибкаHTTP = "";
	Результат = ОбщегоНазначения.ВыполнитьЗапрос(СткЗапроса,ТекстОшибки);
	Возврат Результат;
КонецФункции	
...
Функция Регл_ОбработкаСпискаПользователей() Экспорт
	...
	Запрос.Текст = 
		...
		|	// + Алексей 
		|	ПОДСТРОКА(ИнформационныеБазы.СтрокаСоединенияСигур, 1, 1) <> "" "" КАК ПризнакОбработкаСигур
		|	// - Алексей 
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ПризнакОбработкаСигур Тогда
